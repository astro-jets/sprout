...
const initialCar: CarProps = {
    name: '', description: '', plate: '', path: ''
}
const NewCar = () => {
    ...
    const [formData, setFormData] = useState(initialCar);
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true)
        const res = await fetch(`http://localhost:3000/api/cars/new`, {
            method: "POST",
            body: data,
        });
        console.log("Res => ", res)
    ...
    return (
        <>
            ...
            <SucessModal
                message="Succefully created the Car."
                title="Car created"
                isOpen={showModal}
                onClose={() => {
                    setShowForm(false)
            ...
            <ErrorModal
                message={errMsg}
                title="Failed to create the Car"
                isOpen={showErrModal}
                onClose={() => {
